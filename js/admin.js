...
class AdminPanel {
    constructor() {
        ...
        this.currentEditingUpdate = null;
        this.visitorPage = 1;
    ...
    setupEventListeners() {
        ...
        document.getElementById('createUpdateBtn')?.addEventListener('click', () => this.openUpdateModal());
...
${contact.admin_notes ? `\nAdmin Notes:\n${contact.admin_notes}` : ''}
    ...
    renderUpdates() {
        ...
        if (!this.updates || this.updates.length === 0) {
            container.innerHTML = '<div class="empty-state">No updates yet. Click "+ New Update" to create your first update!</div>';
            return;
        }
            ...
            <table>
                ...
                <tbody>
                    ${this.updates.map(update => {
                        const statusColor = update.published ? '#10b981' : '#6b7280';
                        ...
                        return `
                            <tr>
                                <td>
                                    <div style="font-weight: 500;">${hasImage}${this.escapeHtml(update.title)}</div>
                                    <div style="font-size: 0.8rem; color: #94a3b8;">${this.escapeHtml(plainContent.substring(0, 60))}${plainContent.length > 60 ? '...' : ''}</div>
                                </td>
                                <td>
                                    <span style="display: inline-block; padding: 0.375rem 0.75rem; border-radius: 20px; font-size: 0.8rem; font-weight: 500; background: ${statusColor}22; color: ${statusColor}; border: 1px solid ${statusColor}44;">
                                        ${statusText}
                                ...
                                <td>
                                    <div class="table-actions">
                                        <button class="table-btn view" onclick="adminPanel.editUpdate('${update.id}')">Edit</button>
                                        <button class="table-btn toggle" onclick="adminPanel.toggleUpdatePublish('${update.id}', ${!update.published})">
                                            ${update.published ? 'Unpublish' : 'Publish'}
                                        </button>
                                        <button class="table-btn refund" onclick="adminPanel.deleteUpdate('${update.id}')">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        `;
    ...
    openUpdateModal(update = null) {
        ...
        if (update) {
            ...
            contentDiv.innerHTML = update.content || '';
            document.getElementById('updatePublished').checked = update.published || false;
            
            if (update.image_url) {
                document.getElementById('updateImageUrl').value = update.image_url;
                document.getElementById('previewImg').src = update.image_url;
                document.getElementById('imagePreview').style.display = 'block';
                document.getElementById('imagePlaceholder').style.display = 'none';
            }
            
            this.currentEditingUpdate = update;
        } else {
            title.textContent = 'Create Update';
            document.getElementById('updatePublished').checked = true;
        }
    ...
    async saveUpdate() {
        ...
        let imageUrl = document.getElementById('updateImageUrl').value;
        ...
        try {
            ...
            if (imageInput.files[0]) {
                imageUrl = await this.uploadUpdateImage(imageInput.files[0]);
            }
    ...
    async toggleUpdatePublish(updateId, publish) {
        try {
            const { error } = await supabase
                .from('updates')
...
console.log('%c✓ admin.js loaded successfully', 'color: #10b981; font-weight: 500');
